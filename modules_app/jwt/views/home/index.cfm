<cfoutput>
	<div class="mx-4 xl:mx-64 flex flex-col gap-4 bg-white shadow sm:rounded-lg px-4 py-5 sm:p-6">
		<h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">JWT</h2>
		<div class="sm:flex sm:items-start sm:justify-between">
			<div>
				<h3 class="text-lg leading-6 font-medium text-gray-900">Secured Page Tester</h3>
				<div class="mt-2 text-sm text-gray-500">
					<p>Welcome to the tester page for the JWT module.</p>
					<p>Here you can send a request using the form below to test out your JWTs</p>
				</div>
				<div class="mt-2 text-sm text-gray-500">
					<p>
						To set up JWTs, log in using the
		...
		<form method="POST" action="#event.buildLink( "jwt.Home.secured" )#" onsubmit="send(event,this)" class="w-full sm:flex sm:items-center gap-2">
			<div class="w-full sm:max-w-xs">
				<label for="token" class="sr-only">JWT</label>
				<input type="text" name="token" id="token" class="shadow-sm focus:ring-green-500 focus:border-green-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="JWT">
			</div>
